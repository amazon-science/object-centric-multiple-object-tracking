...
from ocl import feature_extractors, metrics, path_defaults, tree_utils
from ocl.cli import cli_utils, eval_utils, train
from ocl.config.datasets import DataModuleConfig
from ocl.config.metrics import SklearnClusteringConfig
...
class EvaluationClusteringConfig:
    ...
    clusterings: Optional[Dict[str, SklearnClusteringConfig]] = None
    ...
    # Setting this allows to change the plugins (e.g. to change the preprocessing pipeline)
    plugins: Optional[Dict[str, PluginConfig]] = None

    # Path to slot representations
    slots_path: str = path_defaults.OBJECTS
    # Path to feature representations
    features_path: str = f"{path_defaults.FEATURES}.features"
    # Path to slot masks, image shaped
    masks_path: str = f"{path_defaults.OBJECT_DECODER}.masks_as_image"
    # Path to slot masks, but flattened to match the size of features
    masks_flat_path: str = f"{path_defaults.OBJECT_DECODER}.masks"
    # Path to reference masks
    target_masks_path: str = f"{path_defaults.INPUT}.segmentation_mask"
...
class Results:
    ...
    has_background: bool
    matching: str
    clustering: Optional[SklearnClusteringConfig] = None
...
class ExtractDataFromPredictions(pl.callbacks.Callback):
    def __init__(self, paths, output_paths=None, transform=None):
        self.paths = paths
    ...
    def on_predict_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        data = {path: tree_utils.get_tree_element(outputs, path.split(".")) for path in self.paths}

        if self.transform:
            data = self.transform(data)

        for path in self.output_paths:
            self.outputs[path].append(data[path].cpu())


def get_data_from_model(model, datamodule, input_paths, output_paths, transform=None):
    data_extractor = ExtractDataFromPredictions(input_paths, output_paths, transform)
    trainer = pl.Trainer(
        accelerator="auto",
        devices=1,
...
def evaluate(config: EvaluationClusteringConfig):
    ...
    with hydra.initialize_config_dir(config_dir=config_dir):
        ...
        datamodule, model, _ = eval_utils.build_from_train_config(
            train_config, config.checkpoint_path
        )
    ...
    for clustering_name, cluster_fn in cluster_fns.items():
        ...
        for predictions, targets, ids in tqdm.tqdm(
            zip(outputs[config.masks_path], outputs[config.target_masks_path], object_ids),
            total=len(object_ids),
        ):
            metric_hung.update(predictions, targets, ids)
